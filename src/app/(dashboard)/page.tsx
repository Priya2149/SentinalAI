        ...
        <div className="overflow-auto rounded-md border">
          <table className="min-w-[720px] w-full text-sm">
            <thead className="bg-muted/50">
              <tr>
                <Th>User</Th><Th>Model</Th><Th>Latency</Th><Th>Tokens</Th><Th>Cost</Th><Th>Status</Th><Th>At</Th>
              </tr>
            </thead>
            <tbody>
              {latest.map(row => (
                  ...
                  <Td>
                    <span className={
                      row.status === "SUCCESS" ? "text-green-600"
                        : row.status === "FLAGGED" ? "text-amber-600"
                        : "text-red-600"
                    }>
                      {row.status}
                    </span>
                  </Td>
                <Td>{new Date(row.createdAt as unknown as string).toLocaleString()}</Td>
